/**
 * Created by andyzhao on 16-8-14.
 */
...
var balls = [];
const colors = [
  "#33B5E5", "#0099CC", "#AA66CC", "#9933CC", "#99CC00", "#669900", "#FFBB33",
...
function getCurShowTimeSeconds() {
  ...
  var ret = currentTime.getHours() * 3600 + currentTime.getMinutes() * 60 +
...
function updateBalls() {
  ...
  while (balls.length > Math.min(288, cnt)) {
    balls.pop();
  }
}
    ...
    for (var j = 0; j < digit[num][i].length; j++) {
      if (digit[num][i][j] == 1) {
        ...
        cxt.arc(x + j * 2 * (RADIUS + 1) + (RADIUS + 1),
                y + i * 2 * (RADIUS + 1) + (RADIUS + 1), RADIUS, 0,
                2 * Math.PI);
        cxt.closePath();
